...
// Known list servers from live capture
...
// DiscoverServers sends ListFrmRequest to known list servers and returns P2P servers.
func DiscoverServers() ([]P2PServer, error) {
	log.Printf("[discovery] Sending ListFrmRequest to %d list servers", len(ListServers))

	for _, addr := range ListServers {
		servers, err := sendListRequest(addr)
		if err != nil {
			log.Printf("[discovery] ListFrmRequest to %s failed: %v", addr, err)
			continue
		}
		if len(servers) > 0 {
			return servers, nil
...
func sendListRequest(addr string) ([]P2PServer, error) {
	...
	// Build ListFrmRequest frame
	// From the binary: iv_init_frm_listsrv builds this
	// The frame format uses GUTES framing (seq, checksum, type)
	// We send a minimal probe to see what the server expects
...
// This is a best-guess based on the GUTES protocol analysis.
// We'll refine it based on actual server responses.
func buildListFrmRequest() []byte {
	buf := make([]byte, 32)
	buf[0] = 0x00
	buf[1] = 0x01 // Frame type guess
	binary.BigEndian.PutUint16(buf[2:4], 28)
	binary.BigEndian.PutUint32(buf[4:8], 1) // sequence
	buf[12] = 0x00                          // mode=0
	binary.BigEndian.PutUint16(buf[14:16], 51701)
	return buf
}
...
// sendDetectReq sends a 68-byte detection request to a P2P server.
// From logs: gat_send_detecReq_2_allp2psrv: len=68
func sendDetectReq(server *P2PServer) error {
	...
	// Build 68-byte detect request
	// From iv_gute_init_frm_detect_req
	// Format is still being reverse-engineered
	frame := make([]byte, 68)
	frame[0] = 0x00
	frame[1] = 0x02 // detect request type guess
	...
	log.Printf("[detect] Got %d byte response from %s: %x", n, server, buf[:min(n, 32)])
	return nil
}
